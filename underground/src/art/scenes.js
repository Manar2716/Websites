...
export const scenes = {
  ...
  beans(ctx, w, h, rng) {
    frame(ctx, w, h, [1250, 1000], [900, 1100], 0.5, (W, H) => {
      room(ctx, W, H, rng, { unit: H * 0.06, glow: 0.3 })
      counter(ctx, W, H, H * 0.42, { tilt: 0.01, front: false })
      ctx.save()
      ctx.globalCompositeOperation = 'screen'
      pool(ctx, W * 0.42, H * 0.52, W * 0.85, IN.brassLit, 0.24)
      ctx.restore()

      // A heap: dense in the middle, thinning at the edges, and lit from
      // the upper left so the pile has a shadow side.
      const cx = W * 0.5
      const cy = H * 0.66
      for (let i = 0; i < 260; i++) {
        const t = Math.sqrt(rng())
        const th = rng() * TAU
        const x = cx + Math.cos(th) * W * 0.42 * t
        const y = cy + Math.sin(th) * H * 0.15 * t - (1 - t) * H * 0.05
        const r = W * 0.033 * (0.7 + rng() * 0.55)
        const shade = Math.max(0, Math.min(1, (x - cx) / (W * 0.45) * 0.5 + 0.5))
        ctx.save()
        ctx.translate(x, y)
        ctx.rotate(rng() * TAU)
        ellipse(ctx, 0, 0, r, r * 0.66)
        const tone = mix('#d6cf90', '#8e8548', shade * 0.6 + rng() * 0.35)
        const bg = ctx.createLinearGradient(-r, -r, r, r)
        bg.addColorStop(0, mix(tone, IN.brassLit, 0.34 * (1 - shade)))
        bg.addColorStop(1, mix(tone, IN.black, 0.3 + shade * 0.34))
        ctx.fillStyle = bg
        ctx.fill()
        ctx.beginPath()
        ctx.moveTo(-r * 0.76, 0)
        ctx.quadraticCurveTo(0, r * 0.2, r * 0.76, 0)
        ctx.strokeStyle = a(IN.black, 0.45)
        ctx.lineWidth = r * 0.14
        ctx.stroke()
        ctx.restore()
      }

      // A scoop, half buried in it
      ctx.save()
      ctx.translate(W * 0.74, H * 0.6)
      ctx.rotate(-0.42)
      ctx.beginPath()
      ctx.ellipse(0, 0, W * 0.13, W * 0.1, 0, 0, TAU)
      const sg = ctx.createLinearGradient(-W * 0.13, -W * 0.1, W * 0.13, W * 0.1)
      sg.addColorStop(0, '#c8c2b6')
      sg.addColorStop(0.45, '#6e6963')
      sg.addColorStop(1, '#26231f')
      ctx.fillStyle = sg
      ctx.fill()
      ctx.beginPath()
      ctx.ellipse(0, W * 0.012, W * 0.105, W * 0.078, 0, 0, TAU)
      ctx.fillStyle = '#171412'
      ctx.fill()
      ctx.beginPath()
      ctx.roundRect(W * 0.1, -W * 0.022, W * 0.2, W * 0.044, W * 0.02)
      ...
      ctx.save()
      ctx.globalCompositeOperation = 'screen'
      pool(ctx, W * 0.34, H * 0.5, W * 0.7, IN.brassLit, 0.16)
      ctx.restore()
    })
  },
  ...
  roast(ctx, w, h, rng) {
    frame(ctx, w, h, [1250, 1000], [900, 1100], 0.5, (W, H) => {
      room(ctx, W, H, rng, { unit: H * 0.05, wallTint: '#2b1a10', glow: 0.1 })
      ...
      // Drum face
      ellipse(ctx, cx, cy, r * 1.2, r * 1.2)
      ctx.fillStyle = '#161311'
      ctx.fill()
      ctx.strokeStyle = a(IN.brass, 0.6)
      ctx.lineWidth = Math.max(2, W * 0.014)
      ctx.stroke()
      for (let i = 0; i < 14; i++) {
        const th = (i / 14) * TAU
        ctx.beginPath()
        ctx.arc(cx + Math.cos(th) * r * 1.05, cy + Math.sin(th) * r * 1.05, W * 0.011, 0, TAU)
        ctx.fillStyle = a(IN.brass, 0.65)
        ctx.fill()
      }
      ...
      ctx.clip()
      const fg = ctx.createRadialGradient(cx - r * 0.1, cy + r * 0.2, 0, cx, cy, r)
      fg.addColorStop(0, '#ffc25c')
      fg.addColorStop(0.32, '#d4562a')
      fg.addColorStop(0.7, '#8a2c12')
      fg.addColorStop(1, '#260b05')
      ctx.fillStyle = fg
      ...
      for (let i = 0; i < 90; i++) {
        ...
        ctx.save()
        ctx.translate(cx + Math.cos(th) * rr, cy + Math.sin(th) * rr)
        ctx.rotate(rng() * TAU)
        ellipse(ctx, 0, 0, W * 0.026, W * 0.017)
        ctx.fillStyle = a('#2a1408', 0.55 + rng() * 0.45)
        ctx.fill()
        ctx.beginPath()
        ctx.moveTo(-W * 0.02, 0)
        ctx.quadraticCurveTo(0, W * 0.005, W * 0.02, 0)
        ctx.strokeStyle = a('#e0913f', 0.3)
      ...
      // Trier handle, bottom left of the face
      ctx.save()
      ctx.translate(cx - r * 0.9, cy + r * 0.86)
      ctx.rotate(0.5)
      ctx.beginPath()
      ctx.roundRect(-W * 0.02, -W * 0.11, W * 0.04, W * 0.22, W * 0.016)
      ...
      ctx.save()
      ctx.globalCompositeOperation = 'screen'
      pool(ctx, cx, cy, r * 2.6, IN.ember, 0.24)
      ctx.restore()
      motes(ctx, cx, cy - r, W * 0.5, H * 0.5, rng, 40)
    })
  },

  grind(ctx, w, h, rng) {
    frame(ctx, w, h, [1250, 1000], [900, 1100], 0.5, (W, H) => {
      room(ctx, W, H, rng, { unit: H * 0.06, glow: 0.18 })
      counter(ctx, W, H, H * 0.76, { tilt: 0.008, front: false })

      const cx = W * 0.5
      const base = H * 0.76

      // Hopper, with beans in it
      ctx.beginPath()
      ctx.moveTo(cx - W * 0.19, H * 0.09)
      ctx.lineTo(cx + W * 0.19, H * 0.09)
      ctx.lineTo(cx + W * 0.1, H * 0.3)
      ctx.lineTo(cx - W * 0.1, H * 0.3)
      ctx.closePath()
      ...
      for (let i = 0; i < 120; i++) {
        ctx.save()
        ctx.translate(cx + (rng() - 0.5) * W * 0.36, H * 0.15 + rng() * H * 0.15)
        ctx.rotate(rng() * TAU)
        ellipse(ctx, 0, 0, W * 0.024, W * 0.016)
        ctx.fillStyle = mix('#54290f', '#1c0e06', rng())
        ctx.fill()
        ctx.restore()
      }
      ctx.restore()
      ...
      // Body — a machined column, not a grey box
      ctx.beginPath()
      ctx.roundRect(cx - W * 0.15, H * 0.3, W * 0.3, H * 0.32, W * 0.02)
      const bg = ctx.createLinearGradient(cx - W * 0.15, 0, cx + W * 0.15, 0)
      bg.addColorStop(0, '#2f2b28')
      bg.addColorStop(0.24, '#6d6459')
      bg.addColorStop(0.62, '#33302c')
      bg.addColorStop(1, '#151312')
      ctx.fillStyle = bg
      ctx.fill()
      // Collar and dial
      ctx.beginPath()
      ctx.roundRect(cx - W * 0.16, H * 0.33, W * 0.32, H * 0.03, W * 0.008)
      ctx.fillStyle = a(IN.brass, 0.55)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(cx, H * 0.47, W * 0.055, 0, TAU)
      ...
      for (let i = 0; i < 10; i++) {
        const th = (i / 10) * TAU
        ctx.beginPath()
        ctx.moveTo(cx + Math.cos(th) * W * 0.04, H * 0.47 + Math.sin(th) * W * 0.04)
        ctx.lineTo(cx + Math.cos(th) * W * 0.05, H * 0.47 + Math.sin(th) * W * 0.05)
        ctx.strokeStyle = a(IN.cream, 0.35)
        ctx.lineWidth = W * 0.004
        ctx.stroke()
      }

      // Chute, and the dose coming out of it
      ctx.fillStyle = '#0f0d0c'
      ctx.fillRect(cx - W * 0.06, H * 0.62, W * 0.12, H * 0.04)
      for (let i = 0; i < 90; i++) {
        const t = rng()
        ctx.fillStyle = a(mix('#8a5426', '#31190a', rng()), 0.45 + rng() * 0.5)
        ctx.beginPath()
        ctx.arc(cx + (rng() - 0.5) * W * 0.09 * (1 + t), H * 0.66 + t * H * 0.06, W * 0.006 * (0.5 + rng()), 0, TAU)
        ctx.fill()
      }

      // Portafilter waiting under it, handle toward the camera
      ctx.beginPath()
      ctx.roundRect(cx - W * 0.14, base - H * 0.055, W * 0.28, H * 0.045, W * 0.012)
      ctx.fillStyle = '#1d1a18'
      ctx.fill()
      ctx.strokeStyle = a(IN.brass, 0.45)
      ctx.lineWidth = Math.max(1, W * 0.005)
      ctx.stroke()
      ctx.beginPath()
      ctx.roundRect(cx - W * 0.025, base - H * 0.02, W * 0.05, H * 0.09, W * 0.016)
      ctx.fillStyle = '#100e0d'
      ctx.fill()

      ctx.save()
      ctx.globalCompositeOperation = 'screen'
      pool(ctx, cx - W * 0.2, H * 0.36, W * 0.42, IN.brassLit, 0.24)
      ctx.restore()
    })
  },
