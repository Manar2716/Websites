...
(function (global) {
  ...
  function driveCaption(cap, t) {
    ...
    /* A caption anchored at the top of the film has nothing to
       arrive from — the page has not been scrolled yet. It is
       fully in on the first painted frame and only ever leaves,
       which is what makes the hero a title card rather than
       something the visitor has to scroll to find. */
    var inP = from <= 0 ? 1 : M.range(t, from, from + lead);
  ...
  function boot() {
    ...
    var stage = new PEET.Stage(canvas, tier);
    if (!stage.ok) {
    ...
    var timeline = new PEET.Timeline(stage, reduced);
    ...
    function frame(now) {
      ...
      /* ── cull the film when it is not on screen ──
         In the project this renderer came from the film *was* the
         page, so it always had the frame to itself. Here it is the
         first two fifths of a long site, and a six-pass HDR
         pipeline running behind the menu, the map and the footer
         is a laptop fan spinning for something nobody can see.

         The stage is sticky inside `.film`, so it is on screen for
         scroll positions from one viewport above the container to
         the end of its track. Outside that the loop still turns —
         it is what reads the scroll — but nothing is drawn and the
         simulation is not stepped, so the film is exactly where it
         was when you come back up to it. */
      var offscreen = scrollY > filmTop + filmRange + vh * 0.5 ||
                      scrollY < filmTop - vh * 1.5;
      if (offscreen) {
    ...
    if (debug) {
      ...
      /* handles for poking at the film from a console — gated, so
         the normal page exposes nothing on window but PEET */
      global.__film = {
