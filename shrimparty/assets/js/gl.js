...
(function (global) {
  ...
  GL.context = function (canvas, overrides) {
    var opts = {
      ...
      /* The film draws straight to the screen every frame and never
         ...
         it was asked — which renders seven different dishes
         and blits the same picture seven times. */
    ...
    /* Half-float colour targets are what let the film hold real
       highlight range — a wet shell specular under a boil lamp,
       on a matte-black ground, clips instantly in 8-bit. If the extension is
  ...
  /* A second framebuffer over an existing colour texture, with no
     depth attachment.

     Sampling a texture that is attached to the framebuffer you are
     currently drawing into is a feedback loop: undefined results
     and, in practice, an INVALID_OPERATION. The particle pass
     needs exactly that — it draws into the scene colour while
     reading the scene depth to fade softly against what is already
     there. Binding the colour through a framebuffer that does not
     own the depth texture makes the read legal.

     Particles then have no depth test, which turns out not to
     matter: the soft-particle term goes to zero for anything
     behind the scene surface, so occlusion is already handled by
     the fade rather than by the test. */
  GL.colorOnly = function (gl, colorTexture, w, h) {
    var fbo = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
