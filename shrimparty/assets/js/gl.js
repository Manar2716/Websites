/* ═══════════════════════════════════════════════════════════
   ...
   fullscreen triangle. No abstraction beyond what this site
   actually uses; nothing here is general-purpose for its own
   sake.

   One thing here is unusual: `GL.target` can attach a second
   colour buffer holding view distance. Steam has to fade as it
   approaches the rim of the bowl it is rising out of, and
   sampling the depth texture that is currently attached to the
   framebuffer you are drawing into is undefined. Writing
   distance out to a second attachment during the opaque pass
   costs one R16F store per pixel and gives the sprite pass — and
   then the depth of field — a distance buffer it can read.
   ═══════════════════════════════════════════════════════════ */
(function (global) {
  'use strict';
  ...
  GL.target = function (gl, w, h, opt) {
    ...
    /* Second colour attachment: view distance, for soft sprites
       and for the depth of field's circle of confusion. */
    var dist = null;
    if (opt.dist) {
      dist = gl.createTexture();
      ...
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT1, gl.TEXTURE_2D, dist, 0);
    ...
    return {
      fbo: fbo, color: color, depth: depth, dist: dist, w: w, h: h,
      bind: function () {
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
        gl.viewport(0, 0, w, h);
        if (dist) {
      ...
      /* The sprite pass draws into this target's colour buffer
         while *sampling* its distance buffer, to fade a puff out
         as it approaches whatever is behind it. Masking the
         second draw buffer is not enough: a texture that is
         still attached to the bound framebuffer is a feedback
         loop whether or not anything is written to it, and the
         draw is dropped. So the attachment comes off for the
         duration of the pass and `bind` puts it back. */
      bindColorOnly: function () {
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
        gl.viewport(0, 0, w, h);
        if (dist) {
