...
(function (global) {
  ...
  /* remap into 0..1 across [a,b], clamped. The workhorse of the
     whole timeline: every act parameter is a `range` of scroll. */
  ...
  /* ── deterministic random ────────────────────────────────────
     Everything scattered in this film — shell positions, spray,
     droplet targets — is seeded, so a reload gives you the same
     film back rather than a different one. */
  ...
  /* Divergence-free drift. One octave only: this runs per body per
     frame across thousands of bodies, and the extra octaves cost
     more than they read on screen at these speeds. */
  ...
  /* ── spring ──────────────────────────────────────────────────
     A real second-order spring, not an ease. Bodies that arrive
     with one of these carry momentum past the target and settle,
     which is the whole difference between "animated" and "weighs
     something". */
  ...
  /* ── catmull-rom through camera keys ─────────────────────────
     Camera moves are authored as sparse positions; this is what
     makes the path between them a curve instead of a dogleg. */
